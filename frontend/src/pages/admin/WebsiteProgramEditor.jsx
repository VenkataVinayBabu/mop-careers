  ...
  return (
    <form onSubmit={save}>
      ...
      <div className="space-y-5 pb-4">
        ...
        {/*
          * THE DAY-BY-DAY PLANNER USED TO LIVE HERE AND WAS REMOVED (8 Sep 2026).
          *
          * It let someone write each class day — day 1 topic, day 2 topic, all
          * the way to day 45 — so a new batch arrived pre-filled. Nobody was
          * ever going to use it: MOP gives the day-by-day plan out in the
          * brochure and again after a student enrols, and transferring 45 rows
          * into a repeater by hand to duplicate a document that already exists
          * is work with no reader. It also sat inside the marketing-site
          * editor, which was the wrong home for a teaching plan.
          *
          * What is left is the one field that genuinely belongs to a
          * programme: how many days a batch of it runs.
          *
          * NOTHING WAS DELETED. `Program.curriculum` still exists, the API
          * still accepts and returns it, and any programme that already has
          * planned days keeps them and still builds batches from them — this
          * form simply passes the existing value straight back. Restoring the
          * editor means bringing back the `CurriculumTemplate` component from
          * git history and one <CurriculumTemplate> tag here.
          *
          * A batch of a programme with no planned days gets days titled "To be
          * announced" for the teacher to fill in as they teach — see
          * backend/app/curriculum.py.
          */}
        <Section
          title="Batch length"
          caption="Internal — this does not appear on the public site."
        >
          <div className="grid gap-5 sm:grid-cols-[12rem_1fr] sm:items-start">
            ...
            <p className="rounded-lg bg-navy-50 p-4 text-xs text-navy-500 sm:mt-6">
              Applies to the <strong>next</strong> batch created. Batches already running
              keep the days they were created with, along with their dates, recordings and
              attendance — changing this never touches a class in progress.
            </p>
          </div>
        </Section>
